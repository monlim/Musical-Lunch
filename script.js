...
const static = new Tone.Player("https://monlim.github.io/Musical-Lunch/Audio/Static.mp3", {volume: -12});
...
const grace = new Tone.Player("https://monlim.github.io/Musical-Lunch/Audio/Manners.mp3");
...
const tigerScale = {0: ["A0", "A1", "A#2", "A#3"], 1: ["C1", "C#2", "C3", "B3"], 2: ["A1", "F#2", "D3", "B3"], 
...
let scaleIndex=0, noteIndexLeft=0.5, noteIndexRight=0.5, pianoVelLeft=0.5, pianoVelRight=0.5, mouthLine=3, synthDurLeft=0.5, synthDurRight=0.5;
let tigerVelLeft=0, tigerVelRight=0;
...
/*const bassLoop = new Tone.Loop((time) => {
  piano.triggerAttackRelease("D2", 0.5, time, 1);
}, "4n");*/

//create midi note loop
const leftTigerLoop = new Tone.Loop((time) => {
  piano.triggerAttackRelease(tigerScale[noteIndexLeft], 0.25, time, tigerVelLeft);
}, "16n");

//create midi note loop
const rightTigerLoop = new Tone.Loop((time) => {
  piano.triggerAttackRelease(tigerScale[noteIndexRight], 0.25, time, tigerVelRight);
}, "16n");
...
function changeScale(){
  if (scaleIndex < 2){scaleIndex = scaleIndex + 1}
    else {scaleIndex = 0};
};
...
let t2on = false;
function Trigger2(rightThumbX, rightPinkyX) {
    if ((rightPinkyX - rightThumbX) <= DistanceActivate){
      if(t2on)return;
      changeScale();
      console.log("right reversed");
      t2on = true;
    };
    if((rightPinkyX - rightThumbX) > DistanceDeactivate){
      t2on = false;
    };
};*/
...
function onResults(results) {
  ...
  if(results.faceLandmarks){
    ...
    pingPong.wet.value = dist.wet.value = scaleValue(verticalMouth, [6, 12], [0, 1]);
    grace.volume.value = powerScale(verticalMouth);
    static.playbackRate = scaleValue(verticalMouth, [0, 12], [0.2, 2]);
    static.volume.value = scaleValue(verticalMouth, [0, 12], [-25, -12]);
    reverb.wet.value = scaleValue(verticalMouth, [0, 12], [0.8, 0]);
    if (verticalMouth > 1){
      static.start();
    } else {
      static.stop();
  ...
  if(results.rightHandLandmarks){
    ...
    tigerVelLeft = scaleValue(leftIndex.y, [0, 0.5], [1, 0]);
    //synthDurLeft = scaleValue(leftIndex.y, [0, 1], [0.8, 0.1]);
    setInterval(leftVelocityCounter(leftIndex.x, leftIndex.y), 100);
    if (leftThumb && leftPinky){Trigger1(leftThumb.x, leftPinky.x)};
  } else {
    leftLoop.stop();
    leftTigerLoop.stop();
  }
  if(results.leftHandLandmarks){
    ...
    //synthDurRight = scaleValue(rightIndex.y, [0, 1], [0.8, 0.1]);
